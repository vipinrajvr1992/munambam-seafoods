(() => {
    ...
    function setActiveOperator(name) {
        ...
        window.dispatchEvent(new CustomEvent("munambam:operator-changed", { detail: { name: value } }));
    }
    ...
    function clearActiveOperator() {
        ...
        window.dispatchEvent(new CustomEvent("munambam:operator-changed", { detail: { name: "" } }));
    }
    ...
    /* A settings PIN unlock is valid only during the current page visit.
     * Never carry an unlocked Settings state into a fresh reload. */
    sessionStorage.removeItem(PIN_UNLOCK_KEY);
