(() => {
    ...
    const MODULES = {
        ...
        settings: {
            title: "Settings",
            subtitle: "Secure store, delivery, tax and notification settings",
            table: null,
            columns: []
        }
            ...
            <div class="module-page">
                ...
                <div class="module-toolbar">
                    ...
                    <div class="module-toolbar-controls">
                        <select id="moduleSort" class="module-sort" aria-label="Sort records">
                            <option value="__default__">Current order</option>
                        </select>
                        <button type="button" class="module-btn secondary module-select-all-filtered" id="selectAllFiltered">
                            Select all
                        </button>
                        <button type="button" class="module-btn secondary module-clear-selection" id="clearSelection">
                            Clear
                        </button>
                        <span class="module-count" id="moduleCount">—</span>
                        <span class="module-selection-count" id="moduleSelectionCount">0 selected</span>
                    </div>
    ...
    async function openModule(
        ...
        /*
         * Settings is a protected, dedicated form.
         * It must never fall through to the generic table renderer.
         */
        if (section === "settings") {
        ...
        $("moduleSort")?.addEventListener("change", event => {
            const value = event.target.value || "__default__";
            if (value === "__default__") {
                currentSortKey = "__default__";
                currentSortDirection = "desc";
            } else {
                const [key, direction] = value.split("|");
                currentSortKey = key || "__default__";
                currentSortDirection = direction === "asc" ? "asc" : "desc";
            }
            renderModuleTable();
        });
        ...
        $("selectAllFiltered")?.addEventListener("click", () => {
            ...
            displayed.forEach((row, index) => selectedRowKeys.add(rowKey(row, index)));
            updateExportContext(displayed);
            renderModuleTable();
            toast(`${displayed.length.toLocaleString("en-IN")} visible records selected.`);
        });

        $("clearSelection")?.addEventListener("click", () => {
            selectedRowKeys.clear();
            updateExportContext(getDisplayedRows());
            renderModuleTable();
        });
                                ...
                                .map(
                                    ([key]) => {
                                        ...
                                        if (currentSection === "audit") {
                                            ...
                                            else if(key==="actor_user_id") {
                                                const rowMeta = parseObject(row.metadata);
                                                value = rowMeta.operator_name || "Administrator";
                                            }
                                        }
    ...
    function openAuditHumanView(row) {
        ...
        const ignored = new Set(["id","created_at","updated_at","user_id","actor_user_id","role_code","target_id","entity_id","ip_address","user_agent","device_id","session_id","metadata","raw_response"]);
        ...
        const actorEmail = String(meta.email || meta.auth_email || "").trim();
        ...
        openModal(`
            ...
            <div class="detail-grid audit-summary-grid">
                ...
                ${actorEmail ? `<div class="detail-field"><span>Account</span><strong>${escapeHTML(actorEmail)}</strong></div>` : ""}
            </div>
            <div class="audit-human-changes">
                <div class="settings-section-title">
                    <strong>What changed</strong>
                    <span>Only meaningful changes are shown.</span>
                </div>
                ${changes || `<div class="audit-no-change">No additional field changes were recorded for this activity.</div>`}
            </div>
    ...
    function settings() {
        ...
        client.from("settings").select("*").limit(1).maybeSingle().then(async ({ data, error }) => {
            ...
            $("saveOperatorsBtn")?.addEventListener("click", async () => {
                ...
                window.munambamOperators?.lockSettings?.();
                toast("Operator list saved. Settings are locked again.", "success");
                showSettingsPinGate();
            });
    ...
    function openModal(
        ...
        overlay.innerHTML = `
            ...
            <div
                class="module-modal ${escapeHTML(options.className || "")}"
                role="dialog"
                aria-modal="true"
            >
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            .module-table-scroll.is-dragging {
                cursor: grabbing;
                user-select: none;
            }
            ...
            @media (max-width: 760px) {
                ...
                .module-toolbar-controls .module-btn {
                    flex: 1 1 auto;
                }
