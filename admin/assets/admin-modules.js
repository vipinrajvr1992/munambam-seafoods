(() => {
    ...
    // Backward-compatible alias used by the existing Settings form helpers.
    function esc(value) {
    ...
    // Compatibility formatter used by the protected Settings screen.
    // Keep all date formatting centralized through the existing date() helper.
    ...
    /* Central audit bridge. Keep admin actions non-blocking when logging fails. */
    async function audit(action, module, targetId = null, metadata = {}) {
        try {
            ...
            // Logging must never break Settings or another admin operation.
        }
    }
    ...
    async function openModule(
        ...
        // Settings is a protected, dedicated screen — never run it through
        // the generic CRUD/table loader.
        if (section === "settings") {
    ...
    function openAuditHumanView(row) {
        ...
        const actionMap = { login_success:"Signed in", login_failed:"Sign-in failed", logout:"Signed out", create:"Created", update:"Updated", delete:"Deleted", view:"Viewed", toggle:"Status changed", approve:"Approved", reject:"Rejected", adjust_stock:"Stock adjusted", export:"Report exported", unlock:"Settings unlocked", status:"Status updated" };
        const actionText = actionMap[String(row.action || "").toLowerCase()] || humanLabel(row.action, "Administrative activity");
        const areaText = humanLabel(row.module || row.entity_type, "Admin");
        const operator = meta.operator_name || window.munambamOperators?.getActiveOperator?.() || "Administrator";
        ...
        const changes = keys.map(key => {
            ...
            if (same) return "";
            const field = humanLabel(key);
            const fmt = v => { if (v === true) return "Yes"; if (v === false) return "No"; if (v == null || v === "") return "Not set"; if (typeof v === "object") return JSON.stringify(v); return String(v); };
            return `<div class="audit-change"><strong>${escapeHTML(field)}</strong><span>${escapeHTML(fmt(a))} → ${escapeHTML(fmt(b))}</span></div>`;
        }).filter(Boolean).join("");
        const targetName = after.name || before.name || after.product_name || before.product_name || meta.product_name || "";
        const summary = row.description || (targetName ? `${actionText}: ${targetName}.` : `${actionText} in ${areaText}.`);
        ...
        openModal(`
            ...
            ${changes ? `<div class="audit-human-changes"><div class="settings-section-title"><strong>What changed</strong><span>Only the meaningful changes are shown.</span></div>${changes}</div>` : ""}
        `);
    }
    ...
    function lockAndShowSettingsGate() {
        window.munambamOperators?.lockSettings?.();
        showSettingsPinGate();
    }
    ...
    function showSettingsPinGate() {
        ...
        page?.classList.add("settings-rebuilt");
        page?.querySelector(".module-toolbar")?.setAttribute("hidden", "true");
        $("moduleAdd")?.remove();
        ...
        W.innerHTML = `
            <div class="settings-lock-panel">
                <div class="settings-lock-icon" aria-hidden="true">⌘</div>
                <div class="settings-section-title">
                    <strong>Settings are protected</strong>
                    <span>Enter your 4-digit security PIN to manage store, commerce, operators and security settings.</span>
                </div>
                <form id="settingsPinForm" class="module-form settings-runtime-card settings-pin-card">
                    <label>
                    ...
                    <div class="settings-meta">Settings access is temporary and locks again when you leave, reload, save changes, or sign out.</div>
                </form>
            </div>
        `;
        const form = $("settingsPinForm");
        ...
        form?.addEventListener("submit", async (event) => {
            ...
            try {
                ...
                if (!ok) {
                    ...
                    if (button) { button.disabled = false; button.textContent = "Unlock Settings"; }
                    form.querySelector('input[name="pin"]')?.focus();
                ...
                await renderUnlockedSettings();
            } catch (error) {
                toast(error?.message || "Settings could not be unlocked.", "error");
                if (button) { button.disabled = false; button.textContent = "Unlock Settings"; }
            }
        });
    }
    ...
    async function getSettingsRecord() {
        ...
        const defaults = {
            ...
            admin_name: window.munambamOperators?.getActiveOperator?.() || "Administrator",
            admin_email: "",
            phone: "",
            whatsapp: "",
            address: "",
        ...
        const created = await client.from("settings").insert(defaults).select("*").single();
        if (created.error) throw created.error;
        await audit("create", "settings", created.data?.id || null, { initialized: true });
        return created.data;
    }

    async function renderUnlockedSettings() {
        const page = document.querySelector(".module-page");
        page?.classList.add("settings-rebuilt");
        page?.querySelector(".module-toolbar")?.setAttribute("hidden", "true");
        $("moduleAdd")?.remove();
        ...
        L.hidden = false;
        W.hidden = true;
        L.textContent = "Loading settings…";
        ...
        try {
            ...
            W.hidden = false;
            W.innerHTML = `
                <form id="settingsForm" class="module-form settings-runtime-card settings-full-form">
                    <div class="settings-section-title"><strong>Store Profile</strong><span>Business details shown across the store.</span></div>
                    <div class="form-grid-2">
                        ${F("store_name", "Store Name", data.store_name || "Munambam Seafoods", "text", "required")}
                        ${F("admin_name", "Administrator Name", data.admin_name || activeOp || "Administrator")}
                        ${F("admin_email", "Administrator Email", data.admin_email || "", "email")}
                        ${F("phone", "Phone", data.phone || "", "tel")}
                        ${F("whatsapp", "WhatsApp", data.whatsapp || "", "tel")}
                        ${F("currency", "Currency", data.currency || "INR")}
                    </div>
                    <label><span>Business Address</span><textarea name="address" rows="3">${esc(data.address || "")}</textarea></label>

                    <div class="settings-section-title"><strong>Commerce & Tax</strong><span>Amounts and stock thresholds used by the store.</span></div>
                    <div class="form-grid-2">
                        ${F("tax_rate", "Tax Rate (%)", data.tax_rate ?? 0, "number", 'min="0" max="100" step="0.01"')}
                        ${F("free_delivery_threshold", "Free Delivery Above (₹)", data.free_delivery_threshold ?? 0, "number", 'min="0" step="0.01"')}
                        ${F("low_stock_threshold", "Low Stock Threshold (units)", data.low_stock_threshold ?? 5, "number", 'min="0" step="1"')}
                    </div>

                    <div class="settings-section-title"><strong>Store Behaviour</strong><span>Turn store and notification features on or off.</span></div>
                    <div class="settings-toggle-grid">
                        ...
                        <label class="settings-toggle"><input name="email_notifications" type="checkbox" ${data.email_notifications ? "checked" : ""}><span><strong>Email Notifications</strong><small>Send administrative alerts.</small></span></label>
                        <label class="settings-toggle"><input name="order_notifications" type="checkbox" ${data.order_notifications ? "checked" : ""}><span><strong>Order Notifications</strong><small>Send order activity notifications.</small></span></label>
                    </div>

                    <div class="settings-section-title"><strong>Operators</strong><span>Names available for administrator activity records.</span></div>
                    <div class="detail-grid">
                        <div class="detail-field"><span>Current administrator</span><strong>${esc(activeOp)}</strong></div>
                        <div class="detail-field"><span>Current device</span><strong>${esc(hwid)}</strong></div>
                    </div>
                    <label><span>Operator Names</span><textarea id="operatorsList" name="operators_list" rows="4">${esc(operators.join("\n"))}</textarea></label>
                    <div class="form-actions settings-inline-actions">
                        <button id="saveOperatorsBtn" type="button" class="module-btn secondary">Save Operator Names</button>
                    </div>

                    <div class="settings-section-title"><strong>Security</strong><span>Change the 4-digit PIN that protects this Settings area.</span></div>
                    <div class="form-grid-2">
                        ${F("current_pin", "Current PIN", "", "password", 'inputmode="numeric" maxlength="4" pattern="[0-9]{4}" autocomplete="current-password"')}
                        ${F("new_pin", "New 4-digit PIN", "", "password", 'inputmode="numeric" maxlength="4" pattern="[0-9]{4}" autocomplete="new-password"')}
                    </div>
                    <div class="form-actions settings-inline-actions">
                        <button id="changePinBtn" type="button" class="module-btn secondary">Change PIN</button>
                        <button id="lockSettingsBtn" type="button" class="module-btn secondary">Lock Settings</button>
                    </div>

                    <div class="settings-footer-actions">
                        <span class="settings-meta">Last updated: <strong>${esc(fmtDate(data.updated_at))}</strong></span>
                        <div class="form-actions">
                            <button id="settingsReload" type="button" class="module-btn secondary">Reload</button>
                            <button type="submit" class="module-btn primary">Save Settings</button>
                        </div>
                    </div>
                </form>
            `;
            ...
            $("saveOperatorsBtn")?.addEventListener("click", async () => {
                ...
                await audit("update", "settings", data.id, { operators: list });
                toast("Operator names saved.", "success");
                renderUnlockedSettings();
            });

            $("changePinBtn")?.addEventListener("click", async () => {
                const current = $("settingsForm")?.querySelector('[name="current_pin"]')?.value || "";
                const next = $("settingsForm")?.querySelector('[name="new_pin"]')?.value || "";
                if (!/^\d{4}$/.test(next)) return toast("New PIN must be exactly 4 digits.", "error");
                const ok = await window.munambamOperators?.verifyPin?.(current);
                if (!ok) return toast("Current PIN is incorrect.", "error");
                try {
                    await window.munambamOperators.setPin(next);
                    await audit("update", "settings", data.id, { action: "pin_changed" });
                    toast("Security PIN changed. Settings are locked again.", "success");
                    showSettingsPinGate();
                } catch (e) {
                    toast(e.message || "Could not change the PIN.", "error");
                }
            });
            ...
            $("settingsForm").onsubmit = async event => {
                ...
                const payload = {
                    store_name: String(f.get("store_name") || "Munambam Seafoods").trim(),
                    admin_name: String(f.get("admin_name") || activeOp || "Administrator").trim(),
                    admin_email: String(f.get("admin_email") || "").trim() || null,
                    phone: String(f.get("phone") || "").trim() || null,
                    whatsapp: String(f.get("whatsapp") || "").trim() || null,
                    address: String(f.get("address") || "").trim() || null,
                    currency: String(f.get("currency") || "INR").trim() || "INR",
                    tax_rate: Number(f.get("tax_rate") || 0),
                    free_delivery_threshold: Number(f.get("free_delivery_threshold") || 0),
                    low_stock_threshold: Math.max(0, Math.floor(Number(f.get("low_stock_threshold") || 0))),
                ...
                const r = await client.from("settings").update(payload).eq("id", data.id);
                if (r.error) return toast(r.error.message, "error");
                await audit("update", "settings", data.id, { changed: Object.keys(payload) });
                window.munambamOperators?.lockSettings?.();
            ...
            W.hidden = false;
            W.innerHTML = `
                <div class="module-error settings-error-card">
                    <strong>Settings could not be opened</strong>
                    <p>${esc(error?.message || "The settings record could not be loaded.")}</p>
                    <p class="settings-meta">Check that the authenticated administrator has access to the <strong>settings</strong> table.</p>
                    <button type="button" class="module-btn primary" id="settingsRetry">Try Again</button>
                    <button type="button" class="module-btn secondary" id="settingsRelock">Lock Settings</button>
                </div>
            `;
            $("settingsRetry")?.addEventListener("click", () => renderUnlockedSettings());
            $("settingsRelock")?.addEventListener("click", () => lockAndShowSettingsGate());
        }
    }
    ...
    async function settings() {
        ...
        if (moduleTitle) moduleTitle.textContent = "Settings";
        const subtitle = $("moduleSubtitle");
        if (subtitle) subtitle.textContent = "Secure store, commerce, operator and security controls";
        const page = document.querySelector(".module-page");
        page?.classList.add("settings-rebuilt");
        page?.querySelector(".module-toolbar")?.setAttribute("hidden", "true");
        $("modulePrimaryAction")?.remove();
        $("moduleRefresh")?.remove();
        lockAndShowSettingsGate();
    }
