(() => {
    ...
    async function product(row = null) {
        // Load existing gallery images + variants when editing
        let gallery = [];
        let variants = [];
        if (row?.id) {
            try {
                const [imgRes, varRes] = await Promise.all([
                    C.from("product_images")
                        .select("*")
                        .eq("product_id", row.id)
                        .order("display_order", { ascending: true }),
                    C.from("product_variants")
                        .select("*")
                        .eq("product_id", row.id)
                        .order("weight_grams", { ascending: true })
                ]);
                if (!imgRes.error) gallery = imgRes.data || [];
                if (!varRes.error) variants = varRes.data || [];
            } catch (_) {
                // Non-blocking — form still opens with empty extras
            }
        }

        const extraImages = gallery
            .filter(g => !g.is_primary && g.image_url)
            .map(g => g.image_url)
            .slice(0, 4);
        while (extraImages.length < 4) extraImages.push("");

        const weightSlots = [100, 250, 500];
        const variantByWeight = Object.fromEntries(
            variants.map(v => [Number(v.weight_grams), v])
        );

        const imgPreview = (url, id) => `
            <div class="product-img-preview" id="${id}" style="margin-top:8px;${url ? "" : "display:none"}">
                <img src="${esc(url || "")}" alt="" style="max-width:120px;max-height:90px;border-radius:10px;object-fit:cover;border:1px solid rgba(0,0,0,.08)" onerror="this.parentElement.style.display='none'">
            </div>
        `;

        const variantFields = weightSlots.map(w => {
            const v = variantByWeight[w] || {};
            return `
                <div class="settings-section-title" style="margin-top:12px">
                    <strong>${w}g variant</strong>
                    <span>Price, MRP and stock for ${w}g pack</span>
                </div>
                <div class="form-grid-2">
                    ${F(`price_${w}`, "Selling Price (₹)", v.price ?? "", "number", 'min="0" step="0.01"')}
                    ${F(`mrp_${w}`, "MRP / Compare (₹)", v.mrp ?? v.compare_price ?? "", "number", 'min="0" step="0.01"')}
                    ${F(`stock_${w}`, "Stock Qty", v.stock_quantity ?? 0, "number", 'min="0" step="1"')}
                    ${F(`sku_${w}`, "SKU", v.sku || "", "text", 'placeholder="AUTO if blank"')}
                    <input type="hidden" name="variant_id_${w}" value="${esc(v.id || "")}">
                </div>
            `;
        }).join("");
        ...
        modal(`
            ...
            <form id="moduleForm" class="module-form" style="max-height:78vh;overflow:auto">
                <div class="settings-section-title"><strong>Basic info</strong><span>Name, slug, category and descriptions</span></div>
                ${F("name", "Name", row?.name || "", "text", "required")}
                ${F("slug", "Slug", row?.slug || "", "text", "required")}
                ${F("category", "Category", row?.category || "", "text", 'placeholder="e.g. Dried Seafood, Prawns, Podi"')}
                <label><span>Short Description</span><textarea name="short_description" rows="2">${esc(row?.short_description || "")}</textarea></label>
                <label><span>Description</span><textarea name="description" rows="4">${esc(row?.description || "")}</textarea></label>
                <div class="form-grid-2">
                ...
                <div class="settings-section-title"><strong>Photos</strong><span>Main photo + up to 4 additional gallery photos (image URLs)</span></div>
                ${F("main_image_url", "Main photo URL", row?.main_image_url || "", "url", 'placeholder="https://…/main.webp"')}
                ${imgPreview(row?.main_image_url || "", "preview_main")}
                ${[0, 1, 2, 3].map(i => `
                    ${F(`gallery_${i}`, `Additional photo ${i + 1}`, extraImages[i] || "", "url", `placeholder="https://…/photo-${i + 1}.webp"`)}
                    ${imgPreview(extraImages[i] || "", `preview_gallery_${i}`)}
                `).join("")}
                <p class="muted" style="font-size:.75rem;margin:4px 0 0">Tip: host images on Supabase Storage or CDN, then paste the public URL here. Empty fields are ignored.</p>

                <div class="settings-section-title"><strong>Pricing & stock</strong><span>100g · 250g · 500g pack variants (leave price blank to skip a size)</span></div>
                ${variantFields}
                <div class="form-grid-2" style="margin-top:8px">
                    ${F("gst_rate", "GST rate (%)", variants[0]?.gst_rate ?? 5, "number", 'min="0" max="100" step="0.01"')}
                    ${F("hsn_code", "HSN code", variants[0]?.hsn_code || "0306", "text")}
                </div>

                <div class="form-actions">
                    <button class="module-btn secondary" type="button" data-close-modal>Cancel</button>
                    <button class="module-btn primary" type="submit">${row ? "Save Changes" : "Create Product"}</button>
                </div>
            </form>
        `);

        // Live image previews
        const bindPreview = (inputName, previewId) => {
        ...
        bindPreview("main_image_url", "preview_main");
        [0, 1, 2, 3].forEach(i => bindPreview(`gallery_${i}`, `preview_gallery_${i}`));
        ...
        $("moduleForm").onsubmit = async event => {
            ...
            try {
                const mainUrl = (f.get("main_image_url") || "").trim() || null;
                ...
                // Resolve product id (RPC may return id, or we keep existing)
                let productId = row?.id || null;
                if (r.data != null) {
                    if (typeof r.data === "string") productId = r.data;
                    else if (typeof r.data === "object") {
                        productId = r.data.id || r.data.product_id || productId;
                        if (Array.isArray(r.data) && r.data[0]) {
                            productId = r.data[0].id || r.data[0].product_id || productId;
                        }
                    }
                }
                if (!productId) {
                    const lookup = await C.from("products")
                        .select("id")
                        .eq("slug", f.get("slug"))
                        .maybeSingle();
                    productId = lookup.data?.id || null;
                }
                if (!productId) {
                    toast("Product saved, but could not resolve id for images/pricing.", "error");
                    $("munambamModuleModal")?.remove();
                    await load();
                    return;
                }

                // —— Gallery images (main + up to 4 additional) ——
                const galleryUrls = [0, 1, 2, 3]
                    .map(i => (f.get(`gallery_${i}`) || "").trim())
                    .filter(Boolean);

                // Replace gallery rows for this product
                const delImg = await C.from("product_images").delete().eq("product_id", productId);
                if (delImg.error && !String(delImg.error.message || "").includes("0 rows")) {
                    // Soft-fail: continue; main image is already on products table
                    console.warn("product_images delete:", delImg.error);
                }
                ...
                if (imageRows.length) {
                    const insImg = await C.from("product_images").insert(imageRows);
                    if (insImg.error) {
                        toast("Product saved, but gallery images failed: " + insImg.error.message, "error");
                    }
                }

                // —— Variants / pricing ——
                const gstRate = Number(f.get("gst_rate") || 5);
                const hsn = (f.get("hsn_code") || "0306").trim();
                const slugBase = String(f.get("slug") || "product")
                    .toUpperCase()
                    .replace(/[^A-Z0-9]+/g, "-")
                    .replace(/^-|-$/g, "");

                for (const w of weightSlots) {
                    const priceRaw = f.get(`price_${w}`);
                    const price = priceRaw === "" || priceRaw == null ? null : Number(priceRaw);
                    const mrpRaw = f.get(`mrp_${w}`);
                    const mrp = mrpRaw === "" || mrpRaw == null ? null : Number(mrpRaw);
                    const stock = Number(f.get(`stock_${w}`) || 0);
                    const skuInput = (f.get(`sku_${w}`) || "").trim();
                    const existingId = (f.get(`variant_id_${w}`) || "").trim() || null;
                    const variantName = `${w}g`;
                    const sku = skuInput || `${slugBase}-${w}G`;

                    if (price == null || !Number.isFinite(price)) {
                        // No price → remove existing variant for this weight if any
                        if (existingId) {
                            await C.from("product_variants").delete().eq("id", existingId);
                        }
                        continue;
                    }
                    ...
                    if (existingId) {
                        const up = await C.from("product_variants")
                            .update(payload)
                            .eq("id", existingId);
                        if (up.error) {
                            toast(`${w}g variant update failed: ${up.error.message}`, "error");
                        }
                    } else {
                        // Upsert by product_id + weight if unique constraint exists
                        const existing = await C.from("product_variants")
                            .select("id")
                        ...
                        if (existing.data?.id) {
                            ...
                            if (up.error) toast(`${w}g variant update failed: ${up.error.message}`, "error");
                        } else {
                            const ins = await C.from("product_variants").insert({
                                ...payload,
                                created_at: new Date().toISOString()
                            });
                            if (ins.error) toast(`${w}g variant create failed: ${ins.error.message}`, "error");
                        }
                    }
                }

                await audit(row ? "update" : "create", "products", productId, {
                    main_image: Boolean(mainUrl),
                    gallery_count: galleryUrls.length,
                    variants: weightSlots.filter(w => {
                        const p = f.get(`price_${w}`);
                        return p !== "" && p != null && Number.isFinite(Number(p));
                    })
                });
                $("munambamModuleModal")?.remove();
                toast(row ? "Product updated (photos + pricing)." : "Product created (photos + pricing).", "success");
                await load();
            } catch (err) {
                ...
                if (submitBtn) {
                    ...
                    submitBtn.textContent = row ? "Save Changes" : "Create Product";
                }
            }
        };
    }
    ...
    async function deleteRecord(row) {
        ...
        if (section === "coupons") {
            ...
            // Clean related images & variants first (ignore errors if FK cascade exists)
            await C.from("product_images").delete().eq("product_id", row.id);
            await C.from("product_variants").delete().eq("product_id", row.id);
