(() => {
    ...
    async function loadTopProducts() {
        ...
        for (const row of data || []) {
            const current = map.get(row.product_name) || {
                qty: 0,
                total: 0
            };
    ...
    async function loadRecentOrders() {
        ...
        $("recentOrders").innerHTML = data?.length
            ? data.map(row => `
                <div class="list-row">
                    <div class="row-main">
                        <strong>#${escapeHtml(row.order_number)}</strong>
                        <span>${new Date(row.created_at).toLocaleString("en-IN", {
                            day:"2-digit",
                            month:"short",
                            hour:"2-digit",
                            minute:"2-digit"
                    ...
                    <span class="status-badge ${safeClass(row.order_status)}">
                        ${escapeHtml(row.order_status)}
                    </span>
                </div>
            `).join("")
    ...
    async function loadStockAlerts() {
        ...
        if (!data?.length) {
            $("stockAlerts").innerHTML =
                `<div class="empty">No inventory alerts yet.</div>`;
            return;
        }
            ...
            document.addEventListener(
                ...
                () => {
                    ...
                    document.body.style.cursor =
                        'url("data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 width=%2732%27 height=%2732%27 viewBox=%270 0 32 32%27%3E%3Cg transform=%27rotate(-8 16 16)%27%3E%3Cellipse cx=%2714%27 cy=%2717%27 rx=%2710%27 ry=%276%27 fill=%27%231769e8%27/%3E%3Cpath d=%27M23 17l7-6v12z%27 fill=%27%230d4fc4%27/%3E%3Ccircle cx=%2710%27 cy=%2715%27 r=%271.4%27 fill=%27white%27/%3E%3C/g%3E%3C/svg%3E") 8 8, auto';
                }
            );
        });
    }
    ...
    async function init() {
        try {
            ...
            /*
             * Navigation is handled exclusively by
             * admin-modules.js.
             *
             * The old dashboard navigation handler
             * has intentionally been removed because
             * it blocked all modules except Overview.
             */
