(() => {
    ...
    async function loadSales() {
        ...
        if (range === "today") {
            ...
            const customStart = new Date(fromValue);
            const customEnd = new Date(toValue);
        ...
        const hourly =
            range === "today" ||
            (range === "custom" && duration <= 24 * 60 * 60 * 1000);
        ...
        for (
            ...
            d <= bucketEnd && buckets.length < 366;
            d = new Date(d.getTime() + bucketMs)
        ) {
            buckets.push({
                key: d.toISOString(),
                value: 0,
        ...
        for (const row of data || []) {
            ...
            const target = bucketMap.get(bucket.toISOString());
            if (target) {
                target.value += Number(row.total_amount || 0);
            }
        }
    ...
    function setupSalesRange() {
        ...
        select.addEventListener("change", async () => {
            ...
            try {
                await loadSales();
                toast("Sales range updated.", "success");
            } catch (error) {
                console.error(error);
                toast(error.message || "Unable to update sales range.");
            }
        });

        apply.addEventListener("click", async () => {
            if (!from.value || !to.value) {
                toast("Select From and To date/time first.");
                return;
            }
            ...
            if (
                ...
                toast("From date/time must be before To date/time.");
                return;
            }

            try {
                await loadSales();
                toast("Custom sales range applied.", "success");
            } catch (error) {
                console.error(error);
                toast(error.message || "Unable to apply sales range.");
            }
        });
