(() => {
    ...
    async function requireAdmin() {
        ...
        if (avatar) {
            const label = operator || user.user_metadata?.full_name || user.email || "A";
            avatar.textContent = String(label).trim().charAt(0).toUpperCase() || "A";
        }
        const welcome = document.querySelector("#dashboardContent .hero-row h2");
        if (welcome && operator) welcome.innerHTML = `Welcome back, ${escapeHtml(operator)} <span aria-hidden="true">👋</span>`;
        return user;
    }
