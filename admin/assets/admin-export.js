(() => {
    ...
    async function fetchReport(key) {
        ...
        if (key === "audit" && Array.isArray(window.munambamAuditExportRows)) {
            return window.munambamAuditExportRows;
        }

        if (
            window.munambamModuleExportRows &&
            Array.isArray(window.munambamModuleExportRows[key])
        ) {
            return window.munambamModuleExportRows[key];
        }
    ...
    function exportPrint(label, columns, values) {
        ...
        if (!win) {
            throw new Error(
                "Popup blocked. Allow popups for the admin panel."
            );
        }

        const header = columns.map((column) =>
            `<th>${safeText(column)}</th>`
        ).join("");
...
<head>
<meta charset="utf-8">
<title>${safeText(label)} | Munambam Seafoods</title>
<style>
body{font-family:Arial,sans-serif;margin:25px;color:#111}
h1{margin:0 0 5px}
h2{font-size:13px;color:#667085;margin:0 0 18px}
table{width:100%;border-collapse:collapse;font-size:9px}
th{background:#0b1c30;color:#fff;padding:7px;text-align:left}
td{border:1px solid #d8e0e8;padding:6px;vertical-align:top}
tr:nth-child(even) td{background:#f5f8fb}
</style>
</head>
<body>
<h1>MUNAMBAM SEAFOODS</h1>
<h2>${safeText(label)}</h2>
