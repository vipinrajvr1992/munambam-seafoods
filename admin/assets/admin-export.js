(() => {
    ...
    function openModal() {
        ...
        const ctxRows = ctx?.selectedRows?.length || 0;
        if (scope) scope.value = ctxRows ? "current" : "current";

        const exportSearch = $("exportSearch");
        if (exportSearch) {
            exportSearch.dataset.userEdited = "";
            exportSearch.value = ctx?.search || "";
        }

        const exportSort = $("exportSort");
        if (exportSort) {
            const currentSort = ctx?.sortKey && ctx.sortKey !== "__default__"
                ? `${ctx.sortKey}|${ctx.sortDirection || "desc"}`
                : "__default__";
            exportSort.value = [...exportSort.options].some(o => o.value === currentSort)
                ? currentSort
                : "__default__";
        }
    ...
    function currentContextFor(key) {
        const ctx = window.munambamModuleExportContext;
        return ctx && ctx.section === key ? ctx : null;
    }
    ...
    function ensureExportControls() {
        ...
        if (!$("exportSearch")) {
            ...
            field.innerHTML = `
                <span>FILTER</span>
                <input id="exportSearch" type="search" placeholder="Filter the export data..." autocomplete="off">
            `;
            const scope = $("exportScope");
            body.insertBefore(field, scope?.parentElement?.nextSibling || body.querySelector(".export-note") || null);
        }
        ...
        if (!$("exportSort")) {
            ...
            field.innerHTML = `
                ...
                <select id="exportSort">
                    <option value="__default__">Keep current order</option>
                </select>
            `;
            const search = $("exportSearch");
            body.insertBefore(field, search?.parentElement?.nextSibling || body.querySelector(".export-note") || null);
        }
        ...
        if (sort && sourceRows.length) {
            const keys = Object.keys(sourceRows[0] || {}).filter(key =>
                !["raw_response"].includes(key)
            );
            const options = [
                `<option value="__default__">Keep current order</option>`
            ];
            ...
            keys.forEach(key => {
                ...
                options.push(
                    `<option value="${safeText(key)}|asc">${safeText(label)} — A to Z / Low to High</option>`,
                    `<option value="${safeText(key)}|desc">${safeText(label)} — Z to A / High to Low</option>`
                );
            });
        ...
        const search = $("exportSearch");
        if (search && !search.dataset.userEdited) {
            search.value = currentSearch;
        }

        const scope = $("exportScope");
        if (scope && scope.value === "selected" && !(ctx?.selectedRows?.length)) {
            scope.value = "current";
        }

        const onEdit = () => {
            if (search) search.dataset.userEdited = "1";
            updateExportInfo();
        };
        ...
        if (sort && !sort.dataset.bound) {
            ...
            sort.addEventListener("change", updateExportInfo);
        }

        if (scope && !scope.dataset.bound) {
            scope.dataset.bound = "1";
            scope.addEventListener("change", updateExportInfo);
        }
    }
    ...
    function updateExportInfo() {
        ...
        let scopeText = "current filtered view";
        if (scope === "selected") scopeText = `${selected} selected record${selected === 1 ? "" : "s"}`;
        if (scope === "all") scopeText = `${total} record${total === 1 ? "" : "s"}`;

        const filterText = $("exportSearch")?.value?.trim();
        info.textContent = filterText
            ? `Exporting ${scopeText}, further filtered by "${filterText}".`
            : `Exporting ${scopeText}${scope === "current" ? ` (${current} record${current === 1 ? "" : "s"})` : ""}.`;
    }
    ...
    function exportPrint(label, columns, values) {
        document.getElementById("munambamPrintRoot")?.remove();

        const root = document.createElement("section");
        root.id = "munambamPrintRoot";
        root.setAttribute("aria-hidden", "true");

        const header = columns.map(c => `<th>${safeText(c)}</th>`).join("");
        const body = values.map(row =>
            `<tr>${row.map(value => `<td>${safeText(value)}</td>`).join("")}</tr>`
        ).join("");

        root.innerHTML = `
            <div class="munambam-print-report">
                <div class="munambam-print-brand">MUNAMBAM SEAFOODS</div>
                <h1>${safeText(label)}</h1>
                <p>Generated ${safeText(new Date().toLocaleString("en-IN"))}</p>
                <table>
        ...
        requestAnimationFrame(() => {
            try {
                ...
                cleanup();
                toast("Unable to open the print dialog.", "error");
            }
        });
    }
    ...
    function bind() {
        ...
        reportSelect?.addEventListener("change", () => {
            const ctx = window.munambamModuleExportContext;
            const same = ctx?.section === reportSelect.value;

            if ($("exportSearch")) {
                $("exportSearch").dataset.userEdited = "";
                $("exportSearch").value = same ? (ctx?.search || "") : "";
            }
            if ($("exportScope")) {
                $("exportScope").value = same && (ctx?.selectedRows?.length) ? "current" : "all";
            }

            const sort = $("exportSort");
            if (sort) {
                sort.innerHTML = '<option value="__default__">Keep current order</option>';
            }
